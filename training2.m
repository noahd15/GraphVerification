...
%% Load Data (expects cell arrays: edge_indices, features, labels)
data = load('converted_dataset.mat');
edge_indices = data.edge_indices;  % Each cell is an adjacency matrix
features = data.features;          % Each cell is a feature matrix
labels = data.labels;              % Each cell is a label (assumed scalar: 0 or 1)
...
%% Initialize Weights and Biases (binary classification: 1 output)
W1 = initializeGlorot([size(first_features, 2), 64], 64, size(first_features, 2));
...
Wlin = initializeGlorot([64, 1], 1, 64);  % Final layer: 1 output
blin = 0;
...
for i = 1:total_samples
    ...
    prediction_val = gather(prediction);  
    y_val = gather(y);                    

    if ~isa(prediction_val, 'double')
        prediction_val = double(prediction_val);
    end
    if ~isa(y_val, 'double')
        y_val = double(y_val);
    end

    if prediction_val == y_val
