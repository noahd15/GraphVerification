...
for i = 1:length(seeds)
    ...
    % grab the macro‐averaged numbers from the end of each array
    testPrec = precision(end);
    testRec    = recall(end);
    testF1        = f1(end);
    ...
    labelsTest, YTestClass, ...
    testAcc, testPrec, testRec, testF1 );

    % Save the model and training logs
    save("models/drone_node_gcn_pca_" + string(seed) + ".mat", "testAcc", "parameters", "precision", "recall", "f1", ...
